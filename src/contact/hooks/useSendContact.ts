...
export const useSendContact = () => {
    return useMutation({
        ...
        onSuccess: () => notify.success('Mensaje enviado. Te vamos a responder por email.'),
        onError: (error) => {
            ...
            if (error instanceof AxiosError && error.response?.status === 429) {
                notify.error('Enviaste demasiados mensajes seguidos. Esperá un minuto.')
                return
            }
            notify.error(getApiErrorMessage(error, 'No pudimos enviar tu mensaje'))
        },
    })
}
